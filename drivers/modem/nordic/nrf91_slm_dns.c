...
static void nrf91_slm_xgetaddrinfo_callback(
	struct modem_chat *chat, char **argv, uint16_t argc, void *user_data)
{
...
static int nrf91_slm_xgetaddrinfo(
	struct nrf91_slm_data *data, const char *hostname, int family)
{
	// AT#XGETADDRINFO=<hostname>[,<family>]
	char request[sizeof("AT#XGETADDRINF=##,#") + NET_IPV6_ADDR_LEN];
	const char match[] = "#XGETADDRINFO: ";
	int ret;

	ret = snprintk(request, sizeof(request), "AT#XGETADDRINFO=\"%s\",%d",
		hostname, family);
	if (ret < 0) {
		return ret;
	}
