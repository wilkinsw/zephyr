...
static void nrf91_slm_pipe_callback(struct modem_pipe *pipe,
	...
	if(event == MODEM_PIPE_EVENT_RECEIVE_READY) {
		...
		while (end != NULL) {
			...
			if (memcmp(start, "#XPOLL:", 7) == 0) {
				...
				// │                └ end
				// └ start
				int id = strtoul((char *)start + 7, NULL, 10);
				struct modem_socket *sock = modem_socket_from_id(
					&data->socket_config, id);
				...
				start = ((uint8_t *)memchr(start, ',', len)) + 1;
				...
				LOG_INF("Received %d bytes", pending);
				LOG_HEXDUMP_DBG(start, pending, "received: ");

				ret = ring_buf_put(&data->sock_recv_rb, start, pending);
			}
				...
				// │                       └ end
				// └ start
				int pending = strtoul((char *)start + 11, NULL, 10);
				if (size < pending) {
					LOG_WRN("Lost %d bytes", pending - size);
					pending = size;
				}
...
static void nrf91_slm_xsocket_callback(struct modem_chat *chat, char **argv,
	uint16_t argc, void *user_data)
{
	...
	if (argc == 4) {
		...
		if (sock) {
			ret = modem_socket_id_assign(&data->socket_config,
				sock, atoi(argv[1]));
...
static void nrf91_slm_xconnect_callback(struct modem_chat *chat, char **argv,
	uint16_t argc, void *user_data)
{
	struct nrf91_slm_data *data = (struct nrf91_slm_data *)user_data;
	if (atoi(argv[1]) == 1) {
...
static int nrf91_slm_xconnect(struct nrf91_slm_data *data, const char *ip_str,
	uint16_t port)
{
	// AT#XCONNECT=<url>,<port>
	char request[sizeof("AT#XCONNECT=##,####") + NET_IPV6_ADDR_LEN];
	const char match[] = "#XCONNECT: ";
	int ret;

	ret = snprintk(request, sizeof(request), "AT#XCONNECT=\"%s\",%d",
		ip_str, port);
	if (ret < 0) {
		return ret;
	}
...
static int nrf91_slm_xsend(struct nrf91_slm_data *data, const void *buf,
	size_t len)
{
	// AT#XSEND
...
static int nrf91_slm_xsendto(struct nrf91_slm_data *data, const void *buf,
	size_t len, const char *ip_str, uint16_t port)
{
	// AT#XSEND
	...
	ret = snprintk(request, sizeof(request), "AT#XSENDTO=\"%s\",%d",
		ip_str, port);
	if (ret < 0) {
		return ret;
	}
	...
	if (ret < 0) {
		...
		count = ret;
		LOG_INF("Sent %d bytes to %s:%d", count, ip_str, port);
	}
...
static int nrf91_slm_xrecv(struct nrf91_slm_data *data, int timeout_s,
	int flags)
{
	...
	ret = modem_pipe_transmit(
		data->uart_pipe, chat->delimiter, chat->delimiter_size);
	if (ret < 0) {
		goto reattach;
	}

	ret = k_sem_take(&data->sock_sem, K_SECONDS(timeout_s));
	if (ret < 0) {
...
static int nrf91_slm_xrecvfrom(struct nrf91_slm_data *data, int timeout_s,
	int flags)
{
	...
	ret = snprintk(request, sizeof(request), "AT#XRECVFROM=%d,%d",
		timeout_s, flags);
	if (ret < 0) {
		goto reattach;
	}
...
static int nrf91_slm_xpoll(struct nrf91_slm_data *data,
	struct zsock_pollfd *fds, int nfds, int timeout_ms)
{
...
int nrf91_slm_socket(struct nrf91_slm_data *data, int family, int type,
	int proto)
{
	int ret;
	...
	if (ret < 0) {
		...
		modem_socket_put(&data->socket_config, ret);
		errno = -ret;
		return -1;
	}
...
int nrf91_slm_connect(struct nrf91_slm_data *data, void *obj,
...
ssize_t nrf91_slm_recvfrom(struct nrf91_slm_data *data, void *obj, void *buf,
	size_t max_len, int flags, struct sockaddr *src_addr, socklen_t *addrlen)
{
	struct modem_socket *sock = (struct modem_socket *)obj;
	int ret = 0;
...
ssize_t nrf91_slm_sendto(struct nrf91_slm_data *data, void *obj, const void *buf,
...
int nrf91_slm_poll(struct nrf91_slm_data *data, struct zsock_pollfd *fds,
