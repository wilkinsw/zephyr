...
#define PERIODIC_SCRIPT_TIMEOUT \
	K_MSEC(CONFIG_MODEM_NRF91_SLM_PERIODIC_SCRIPT_MS)
...
{
	int ret = 0;
	if (k_mutex_lock(&data->chat_lock, K_NO_WAIT) == 0) {
...
static void nrf91_slm_delegate_event(struct nrf91_slm_data *data,
...
MODEM_CHAT_SCRIPT_DEFINE(nordic_nrf91_slm_ppp_chat_script,
			 nordic_nrf91_slm_ppp_chat_script_cmds, abort_matches,
			 nrf91_slm_chat_callback_handler, 10);

//*****************************************************************************
// Modem state machine
//*****************************************************************************

static void nrf91_slm_enter_state(
	struct nrf91_slm_data *data, enum nrf91_slm_state state);

static void nrf91_slm_timeout_handler(struct k_work *item)
{
	struct k_work_delayable *dwork = k_work_delayable_from_work(item);
	struct nrf91_slm_data *data =
		CONTAINER_OF(dwork, struct nrf91_slm_data, timeout_work);
...
{
	const struct nrf91_slm_config *config =
		(const struct nrf91_slm_config *)data->dev->config;
...
static void nrf91_slm_idle_event_handler(struct nrf91_slm_data *data,
					      enum nrf91_slm_event evt)
{
	const struct nrf91_slm_config *config =
		(const struct nrf91_slm_config *)data->dev->config;
							 ...
							 enum nrf91_slm_event evt)
{
	...
	switch (evt) {
	case NRF91_SLM_EVENT_SCRIPT_SUCCESS:
		net_if_set_link_addr(data->netif, data->imei,
...
static void nrf91_slm_enter_state(struct nrf91_slm_data *data,
...
static void nrf91_slm_event_handler(struct nrf91_slm_data *data,
					 enum nrf91_slm_event evt)
{
	enum nrf91_slm_state state;
...
static int nrf91_slm_get_signal(const struct device *dev,
...
static int nrf91_slm_get_modem_info(const struct device *dev,
...
static void nrf91_slm_init_pipe(const struct device *dev)
{
	...
	data->uart_pipe = modem_backend_uart_init(
		&data->uart_backend, &uart_backend_config);
}
...
static void nrf91_slm_init_socket_script(const struct device *dev)
{
	...
	modem_chat_script_chat_set_response_matches(&data->sock_script_chat,
						    &data->sock_match, 1);
	...
	modem_chat_script_set_abort_matches(&data->sock_script, abort_matches, sizeof(abort_matches));
	modem_chat_script_set_timeout(&data->sock_script, 10);

	ring_buf_init(&data->sock_recv_rb,
		sizeof(data->sock_recv_rb_buf), data->sock_recv_rb_buf);
}
...
DEVICE_DT_INST_DEFINE(0, nrf91_slm_init, PM_DEVICE_DT_INST_GET(0),
			  &mdata, &mconfig, POST_KERNEL, 99, &nrf91_slm_api);
...
static int offload_connect(
	void *obj, const struct sockaddr *addr, socklen_t addrlen)
{
...
static ssize_t offload_sendmsg(void *obj, const struct msghdr *msg, int flags)
{
	...
	for (int i = 0; i < msg->msg_iovlen; i++) {
		...
		while (len > 0) {
			ret = nrf91_slm_sendto(&mdata, obj, buf, len, flags,
...
static void modem_net_iface_init(struct net_if *iface)
{
	...
	net_if_set_link_addr(
		iface, data->imei, ARRAY_SIZE(data->imei), NET_LINK_UNKNOWN);

	data->netif = iface;

	modem_socket_init(&data->socket_config, &data->sockets[0],
		ARRAY_SIZE(data->sockets), 0, true, &offload_socket_fd_op_vtable);
...
NET_DEVICE_OFFLOAD_INIT(nrf91_slm_net_dev, "nrf91_slm_net_dev", NULL,
			PM_DEVICE_DT_INST_GET(0), &mdata,
			&mconfig, 98, &api_funcs, 1500);

NET_SOCKET_OFFLOAD_REGISTER(nrf91_slm_sock, CONFIG_NET_SOCKETS_OFFLOAD_PRIORITY,
